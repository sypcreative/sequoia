...
if ( ! \class_exists( __NAMESPACE__ . 'PurgeProxyCacheOnDeletedComment' ) ) {
	 ...
	 * @since  0.0.1
	 */
	class PurgeProxyCacheOnDeletedComment extends AbstractEvent {
		use Instance;
		//use HasContainer;
		...
		public function support() {
			...
			return (bool) $option( 'ahsc_purge_page_on_deleted_comment' );
		}
		...
		public function subscribe() {
			\add_action( 'deleted_comment', array( $this, 'ahsc_purge_page_on_deleted_comment' ), 200, 2 );
			\add_action( 'rest_delete_comment', array( $this, 'ahsc_purge_page_on_deleted_comment_rest' ), 200, 3 );
		}
		...
		public function unsubscribe() {
			\remove_action( 'deleted_comment', array( $this, 'ahsc_purge_page_on_deleted_comment' ), 200 );
			\remove_action( 'rest_delete_comment', array( $this, 'ahsc_purge_page_on_deleted_comment_rest' ), 200 );
		}
		 ...
		 * @return void
		 */
		public function ahsc_purge_page_on_deleted_comment( $id, $comment ) {
			...
			if ( $option( 'ahsc_purge_homepage_on_edit' ) ) {
				...
				// Logger.
				$this->log( 'hook::deleted_comment::home' . $target, __NAMESPACE__ . '::' . __FUNCTION__, 'debug' );
				// Logger.
			...
			// Logger.
			$this->log( 'hook::deleted_comment::' . $target, __NAMESPACE__ . '::' . __FUNCTION__, 'debug' );
			// Logger.
		 ...
		 * @return void
		 */
		public function ahsc_purge_page_on_deleted_comment_rest( $comment, $request, $creating ) {
			$this->ahsc_purge_page_on_deleted_comment( $comment->comment_ID, $comment );
		}
	}
}
