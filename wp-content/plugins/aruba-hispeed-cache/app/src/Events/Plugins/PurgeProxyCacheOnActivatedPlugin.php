...
if ( ! \class_exists( __NAMESPACE__ . 'PurgeProxyCacheOnActivatedPlugin' ) ) {

	/**
	 * PurgeProxyCacheOnActivatedPlugin It is the class that handles the event of 'activate_plugin()'
	 *
	 * Is a class that has no real direct use. But it is handled by the core of this plugin.
	 * It is used to queue the action of clearing the proxy cache to the 'activated_plugin' action.
	 *
	 ...
	 * @since  1.2.0
	 */
	class PurgeProxyCacheOnActivatedPlugin extends AbstractPluginEvent {
		use Instance;
		//use HasContainer;
		...
		public function subscribe() {
			\add_action( 'activated_plugin', array( $this, 'ahsc_purge_on_plugin_actions' ), 200, 1 );
		}
		...
		public function unsubscribe() {
			\remove_action( 'activated_plugin', array( $this, 'ahsc_purge_on_plugin_actions' ), 200 );
		}
