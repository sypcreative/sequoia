...
if ( ! \class_exists( __NAMESPACE__ . 'PurgeProxyCacheOnDeletePlugin' ) ) {

	/**
	 * PurgeProxyCacheOnDeactivatedPlugin It is the class that handles the event of 'delete_plugins()'
	 *
	 * Is a class that has no real direct use. But it is handled by the core of this plugin.
	 * It is used to queue the action of clearing the proxy cache to the 'delete_plugin' action.
	 *
	 ...
	 * @since  1.2.0
	 */
	class PurgeProxyCacheOnDeletePlugin extends AbstractPluginEvent {
		use Instance;
		//use HasContainer;
		...
		public function subscribe() {
			\add_action( 'delete_plugin', array( $this, 'ahsc_purge_on_plugin_actions' ), 200, 1 );
		}
		...
		public function unsubscribe() {
			\remove_action( 'delete_plugin', array( $this, 'ahsc_purge_on_plugin_actions' ), 200 );
		}
